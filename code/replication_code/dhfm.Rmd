...
Packages
```{r}
library(R.matlab)  # for readMat() function
library(matlab)    # 
...
# Data
Use the `readMat()` function from the `R.matlab` package to load the `dhfm_data` m-file. The m-file contains the data stored as a cell array. The `readMat()` function converts data stored in a cell array into a list that contains some unnecessary nested lists. The code below fixes this issue.

```{r}
data <- unlist(readMat('~/OneDrive\ -\ University\ of\ Texas\ at\ Arlington/BHDFM/R_code/replication_code/dhfm_data.mat')$bigZ.ns, recursive = FALSE)
data <- lapply(1:length(data), function(i) if(is.list(data[[i]])) unlist(data[[i]], recursive = FALSE) else data[[i]])
```
